...
class PokemonListViewModel @Inject constructor(
        ...
        viewModelScope.launch(Dispatchers.Default) {
            ...
            val results = listToSearch.filter {
                it.pokemonName.contains(
                    query.trim(),
                    ignoreCase = true
                ) || it.number.toString() == query.toString()
            }
            if (isSearchStarting){
    ...
    fun loadPokemonPaginated() {
        viewModelScope.launch {
            ...
            when (val result =
                ...
                is Resources.Error -> {
                    loadError.value = result.err.message ?: "An error ocurred"
                    isLoading.value = false
                }
            }
        }
    ...
    fun calcDominantColor(asset: Bitmap, onFinish: (Color) -> Unit) {
        val bmp = asset.copy(Bitmap.Config.ARGB_8888, true)
        Palette.from(bmp).generate { palette ->
            palette?.dominantSwatch?.rgb?.let { colorValue ->
                onFinish.invoke(Color(colorValue))
            }
